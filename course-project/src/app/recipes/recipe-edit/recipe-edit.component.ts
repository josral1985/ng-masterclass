...
export class RecipeEditComponent implements OnInit {
          ...
          recipeIngredients.push(
            new FormGroup({
              ...
              amount: new FormControl(ingredient.amount, [
                Validators.required,
                Validators.pattern(/^[1-9]+\d*$/), //regex for number greater than 1
              ]),
            })
          );
        }
  ...
  onSubmit() {
    ...
    // );
    if (this.editMode) {
      this.recipeService.updateRecipe(this.id, this.recipeForm.value); // it will automatically set everything up!
    } else {
  ...
  get controls() {
    //to be able to access the controls of the formArray
